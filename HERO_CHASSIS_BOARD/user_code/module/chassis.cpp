...
void Chassis::power_ctrl()
{
    ...
    {	
        ...
        {
            can_receive.can_cmd_super_cap_power((uint16_t)(70),(uint16_t)chassis_power_buff,super_cap_switch);
        }
        else
        {
...
void Chassis::chassis_behaviour_mode_set()
{
    ...
    //遥控器设置模式
    if ((switch_is_up(chassis_RC->rc.s[CHASSIS_MODE_CHANNEL])) && can_receive.chassis_receive.gimbal_turn_flag != 2) //右拨杆上 底盘行为 跟随云台
    {
        chassis_behaviour_mode = CHASSIS_INFANTRY_FOLLOW_GIMBAL_YAW;
    }
    else if ((switch_is_mid(chassis_RC->rc.s[CHASSIS_MODE_CHANNEL])) || can_receive.chassis_receive.gimbal_turn_flag == 2) //右拨杆中 底盘行为 自主运动
    {
